...
export default function Navbar({ linksPosition = "end", backgroundColor = "white", logo = "/placeholder.svg", shopName = "Shop" }) {
  ...
  const navItems = [
    ...
    ...(products?.length ? [{ to: `/w/${name}/products`, label: "Products" }] : []),
    ...(blogs?.length ? [{ to: `/w/${name}/blogs`, label: "Blogs" }] : []),
    ...(services?.length ? [{ to: `/w/${name}/services`, label: "Services" }] : []),
  ]
    ...
    <nav style={{ backgroundColor }} className="navbar h-16">
      <div className="navbar-start">
        ...
        <div className="flex gap-2 items-center">
          ...
          <h1 className="text-2xl font-bold capitalize text-sky-400/65">{shopName}</h1>
        </div>
      </div>
